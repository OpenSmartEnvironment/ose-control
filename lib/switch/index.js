...
exports.init = function() {  // {{{2
  this.on('relay', O.link.bindResp(O, './relay'));
  /**
  ...
  this.ddef = O.new('ose/lib/field/object')('dval', {order: o});
  this.ddef.add('name', 'text', {required: true});
  this.ddef.add('parent', 'lookup');
  this.ddef.add('master', 'lookup', {  // {{{3
    view: 'list',
    ident: {
      scope: 'control',
    },
  ...
  this.ddef.add('debounce', 'millitime');  // {{{3
  /**
   * Debounce timeout
   *
  ...
  this.ddef.add('hold', 'millitime');  // {{{3
  /**
   * Hold timeout
   *
...
exports.homeInit = function(entry) {  // {{{2
  ...
  if (entry.data.master && entry.data.pin) {
...
//    this.socket.target = patch.master;
  ...
  if (this.data.master && this.data.pin) {
