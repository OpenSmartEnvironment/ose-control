...
Consts.green = 'rgb(58, 170, 53)';  // TODO: move to "ose" consts, add other colors
  ...
  filter: function(key, value, params) {
    if (params.role) {
      if (Array.isArray(value)) {
        if (value.indexOf(params.role) < 0) return false;
      } else {
        if (value !== params.role) return false;
      }
    }
