...
exports.updateStateKey = function(key, val) {  // {{{2
  switch (key) {
  case 'power':
    this.find('li.power > div > h3 > span').text(val);
    this.find('li gaia-slider.power').val(val * 100);
    break;
  case 'enabled':
    this.find('li.state > div > p > span:first-child').text(val ? 'Enabled' : 'Disabled');
    break;
  case 'value':
    this.find('li.state > div > p > span:last-child').text(val ? 'on' : 'off');
    break;
  }
};
