 ...
 */
  ...
  if (key in entry.groups) {
    O.log.warn('Overwriting REMOTE group!', {key: key, entry: entry.identify(), group: group});
  }
...
//  console.log('ADD ITEM', group.key, key, data);
  ...
  if (key in group.actions) {
    O.log.warning('Overwriting remote group action', {group: group.key, key: key, data: data});
  }
...
//    console.log('EXEC', action);
    ...
    case 'function':
      ...
      return;
    case 'object':
      if (action.firstOnly && data.count >= 1) {
        return;
      }
      ...
      O.log.error(O.error(entry, 'UNEXPECTED', 'Action without target or command', action));
      return;
    case 'undefined':
    ...
    throw O.error(entry, 'Invalid action', action);
  }
};
