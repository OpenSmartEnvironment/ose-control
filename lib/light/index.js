...
exports.inMap = function(entry, params) {  // {{{2
  if (params.filter) {
...
exports.homeInit = function(entry) {  // {{{2
  ...
  if (entry.dval) {
    if (entry.dval.channels) {
      if (entry.dval.pin) {
        O.log.warning('Ignoring pin!', {entry: entry.identify(), pin: entry.dval.pin});
      }
    ...
    } else if (entry.dval.pin) {
      new Pin(entry, 'single', entry.dval.pin);
    }

    if (entry.dval.switch) {
      // Creates a client socket
      if (Array.isArray(entry.dval.switch)) {
        for (var i = 0; i < entry.dval.switch.length; i++) {
          new Switch(entry, entry.dval.switch[i]);
        }
      } else {
        new Switch(entry, entry.dval.switch);
      }
    }
  }

//  console.log('LIGHT HARBOUR INITED', entry.id, entry.sval);
};
...
//  console.log('LIGHT DELTA', req, this.entry.sval);
  ...
  if (almostOff(e)) {
    ...
    throw O.error(e, 'INVALID', 'Invalid profile on', e.dval.profiles.on);
  }
...
function almostOff(entry) {  // {{{2
  ...
  throw O.error(entry, 'INVALID', 'Invalid main', entry.sval.main);
};
...
//  console.log('SETUP OFF TIMEOUT', val);
  ...
  case 'object':
    ...
    break;
  default:
    O.log.error(this, 'invalidAutoOff', val);
    return;
  }
