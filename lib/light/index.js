 ...
 *
 * @param req {Object} Sets the light to the value specified. If `req` is not an object `req` is replaced with `{type: req, speed: O.consts.lightDimSpeed}`
...
//  console.log('LIGHT SWITCH', req);
  ...
  L.error(socket, O.error(this.entry, 'Invalid arguments', req));
  return;
...
//  console.log('LIGHT DELTA', req, this.entry.state);
  ...
  if (almostOff(e)) {
    ...
    throw O.error(e, 'INVALID', 'Invalid main on', e.data.main.on);
  }
 ...
 */
  ...
  case 'number':
    ...
    L.error(socket, O.error(e, 'Invalid profile "on"!', e.data.main));
    return;
  case 'object':
    for (var key in req) {
...
function setProfile(entry, name, speed, socket) {  // {{{2
  ...
  L.error(socket, O.error(entry, 'Invalid profile!', name));
  return;
