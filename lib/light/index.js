...
//  console.log('LIGHT PROFILE', req);
  ...
  var p = e.data.profiles && e.data.profiles[req.name];
  if (p === undefined) {
    switch (req.name) {
    case 'off':
      p = 0;
...
//  console.log('LIGHT DELTA', req, this.entry.state);
  ...
  if (almostOff(e)) {
    ...
    switch (typeof (e.data.profiles && e.data.profiles.on || 0)) {
    case 'number':
      for (var key in e.channels) {
        L.send(e.channels[key], 'write', {
    ...
    throw O.error(e, 'INVALID', 'Invalid profile on', e.data.profiles.on);
  }
...
//  console.log('SETUP OFF TIMEOUT', data);
  ...
  case 'object':
    ...
    break;
  default:
    O.log.error(this, 'invalidAutoOff', data);
    return;
  }
