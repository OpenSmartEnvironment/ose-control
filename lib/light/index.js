...
exports.homeInit = function(entry) {  // {{{2
  ...
  if (entry.dval) {
    if (entry.dval.channels) {
      if (entry.dval.pin) {
        O.log.warning('Ignoring pin!', {entry: entry.toString(), pin: entry.dval.pin});
      }
...
function onState(val) {  // {{{2
  ...
  throw O.log.error(this, 'invalidMain', main);
};
...
function almostOff(entry) {  // {{{2
  ...
  throw O.log.error(entry, 'INVALID', 'Invalid main', entry.sval.main);
};
