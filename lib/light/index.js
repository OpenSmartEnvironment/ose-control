...
function mainDetail(view, wrap) {  // {{{2
  ...
  view.li('row')
    .h3('Turn', 'stretch')
    .buttons('text', ['off', 'on', 'full'], {relation: 'single'}, function(name) {
      view.post('profile', name);
    })
  ;
}
...
//    wrap.view.val(Boolean(patch));
    ...
    if (wrap.value) {
      ...
      var time = Math.floor((wrap.value.wait - Date.now() + wrap.value.start) / 1000);
