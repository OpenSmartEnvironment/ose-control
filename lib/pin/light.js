...
// Public {{{1
module.exports = function(pin, req, state, resp, socket) {  // {{{2
  switch (req.type) {
  case 'pwm':
    pin.update = updatePwm;
    pin.send = send;

    break;
  case 'dout':
    pin.update = updateDout;
    break;
  default:
    var err = Ose.error(pin, 'INVALID_ARGS', req);
    Ose.link.error(pin, err);
...
function updateDout(req) {  // {{{2
/**
 * This method is assigned to the `Pin` instance and replaces the
 * original `update()` method.  `this` is a `Pin` instance.
 */

  this.type.write(this, req.value ? 1 : 0);
};

function updatePwm(req) {  // {{{2
/**
 * This method is assigned to the `Pin` instance and replaces the
 * original `update()` method.  `this` is a `Pin` instance.
 */
