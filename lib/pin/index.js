...
//  console.log('NEW PIN', req.index, pins.entry.toString());
  ...
  // Setup type and caps {{{3
  this.type = pins.types[req.type];
  switch (O.typeof(pins.caps)) {
  case 'string':
    if (req.type === pins.caps) this.caps = {};
    break;
  case 'array':
    if (req.type in pins.caps) this.caps = {};
    break;
  case 'object':
  ...
  if (! this.caps) {
    O.link.error(socket, O.error(pins, 'Pin with specified capabilities was not found', req));
    return;
  }

  // Setup other properties {{{3
  ...
  this.setup(req, resp, state, function(err) {  // {{{3
