 ...
 * is created. The [pin list] constructor receives a list of pin types
 * and a list of pins that can be controlled.
 *
 ...
 *
 * Based on this request, a new pin is created and set up by the
 * Raspberry Pi entry. As a response to the `registerPin` command, the
 * current state of the given switch is sent to the `open()` handler
 * of the corresponding client socket.
 *
 ...
 *    invoked.
 *
 * Now it is possible to send `update()` requests from the client side
 ...
 *
 * 4. Create a [pin list] instance for each entry in the `homeInit()` method
 *    of the given [entry kind].
 *
 ...
 * Pin index
 *
 * Pin index within owning [pin list]
 *
 * @aliases pinIndex
 * @property index
 * @type String
...
//    console.log('PIN SETUP RESPONSE', that.index, val, err);
    ...
    if (typeof val !== 'number' || isNaN(val)) {
      throw O.error(that, 'Invalid value', val);
    }
 ...
 */
  ...
  if (this.type && this.type.release) {
    this.type.release(this, this.cease);
  }
};
