...
/*
  var li = $('<li>', {id: view.id + key})
    .appendTo(view.$())
    .click(onClick)
  ;
*/

  var li = view.append('li', {pinIndex: key}).on('click', onClick);
  ...
  li.append('div').add([
    view.new('h3').add([
      view.new('span').text(key + ' – ' + pin.type),
      view.new('span').text(pin.flavour ? ' (' + pin.flavour + ')' : '')
    ]),
    view.new('p').text(pin.caption || (pin.entry && pin.entry.id) || '')
  ]);
